...
describe(`integration app suite`, function () {
  ...
  beforeEach(() => {
    app = appModule();
    account = { access_token: process.env.ENV_TEST_TOKEN };
  });
  ...
  it(`should have app config`, async () => {
    const { body: appConfig } = await request(app)
      .get(`/`)
      .expect(200)
      .expect(`Content-Type`, /json/);

    assert.equal(appConfig.name, `Todoist`);
    assert.match(appConfig.description, /Todoist/);
  ...
  //   expect(body).to.be.name("String");
  // });

  // describe("should have validate end-point", async () => {
  //   const { body: appConfig } = await request(app)
  //     .post("/validate")
  //     .expect(200)
  //     .expect(`Content-Type`, /json/)
