...
const defaultSchema = require(`../schema/schema.default.json`);
const userSchema = require(`../schema/schema.users.json`);

const getNotionClient = (account) =>
  new Client({
...
const getFieldAttributes = ({ type }) => {
  ...
  return _.isEmpty(notionType)
    ? { type: `text` }
    : _.cloneDeep(TYPE_MAPPING[notionType]);
};
...
const createSchemaFromDatabase = (db) => {
  ...
  _.keys(db.properties).forEach((key) => {
    ...
    databaseSchema[key.toLowerCase()] = getSchemaField({
      property,
      key,
      databaseName,
    });
  ...
  if (_.isEmpty(databaseSchema.name)) {
    if (!_.isEmpty(title)) {
      databaseSchema.name = getSchemaField({
...
const getDatabaseItem = (db) => {
  const name = _.get(db, `title[0].plain_text`, `Noname`)
    .replace(/[^\w ]+/g, ``)
    .trim();
...
const resolveSchema = async ({ pagination, client, requestedType }) => {
  ...
  return createSchemaFromDatabase(
    await client.databases.retrieve({ database_id: requestedType })
  );
};
